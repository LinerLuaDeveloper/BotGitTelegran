...
def get_mute_info(user_id):
    ...
    cursor = conn.cursor()
    cursor.execute(
        'SELECT username, reason, muted_by, muted_time, unmute_time, violation_count FROM mutes WHERE user_id = ?',
        (user_id,))
...
def check_spam_early(user_id, message_text):
    ...
    if user_message_count[user_id] > 4:  # Более 4 сообщений подряд быстро
        logger.warning(
            f"Пользователь {user_id} отправляет сообщения слишком быстро: {user_message_count[user_id]} сообщений")
        return True
...
def auto_mute_user(user_id, username, reason="Автоматическое обнаружение спама"):
    ...
    try:
        ...
        # Логируем мут
        logger.warning(
            f"Пользователь {user_id} (@{username}) автоматически заглушен на {mute_duration}. Нарушение #{violation_count}")
